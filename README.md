...
The server starts on port 8000 by default. Set the `PORT` environment variable to override. The MCP endpoint is at `/mcp` (streamable HTTP, stateless). Health checks are available at `/` and `/health`.
